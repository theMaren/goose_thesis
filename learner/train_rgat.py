...
from dataset.dataset_rgat import get_loaders_from_args_gnn
...
if __name__ == "__main__":
    ...
    # Replace "gat_max_print" with "training_logs"
    log_save_file = args.save_file.replace("gat_max_print", "training_logs")
    # Replace ".model" with ".txt"
    log_save_file = log_save_file.replace(".model", ".txt")
    print(log_save_file)
        ...
        with open(log_save_file, "w") as log_file:
            for e in range(epochs):
                ...
                if lr < 1e-5:
                    ...
                    log_file.write(f"Early stopping due to small lr: {lr}\n")
                    break
                
    except KeyboardInterrupt:
        print("Early stopping due to keyboard interrupt!")
        with open(log_save_file, "a") as log_file:
            log_file.write("Early stopping due to keyboard interrupt!\n")
